extensions [vid]
;;EDITS
;;Add Network Setup Code
;;Add-Links Method
;;Transmit disease through link-neighbors

globals [count-disease-s count-disease-i count-disease-r percent-infected _recording-save-file-name]
...
to setup
  ...
  ;ask turtles [ add-edge ]
  ;layout-radial turtles links (turtle 0)
  ;repeat 100 [ layout-spring turtles links 0.2 5 1 ]
  ;;;;;;;;;;;;;;;;;;
...
to setup-spatially-clustered-network
  let num-links (average-node-degree * Num-Agents) / 2
  ...
  repeat 10
  [
    layout-spring turtles links 0.3 (world-width / (sqrt Num-Agents)) 1
  ]
end
...
to update-states
  ...
  ;;Spread Disease from Infected Agents.
  ask-concurrent turtles with [disease-state = "I"] [
    ask link-neighbors [
      if (disease-state = "S")[
        if (random-float 1 < Diffusion-Rate)[
          set disease-state "I"
        ]
      ]
    ]
  ...
  ;;Recover/Update All Infecteds.
  ask-concurrent turtles with [disease-state = "I"] [
    ifelse (time-infected >= adoption-duration)
    [
      set disease-state "R"
    ]
    [
...
to update-globals
  ...
  set percent-infected ((count-disease-I + count-disease-R)/ Num-Agents * 100)
end


to do-plots
  set-current-plot "Adoption Curves"
...
Network-Type
Network-Type
"Spatially Clustered" "Random" "Small World"
1
...
This model demonstrates an agent-based implementation of a standard epidemiological SIR model. 

Agents move randomly about the space, contracting / recovering from an infection as specified by the model parameters. 
@#$#@#$#@
default
true
0
